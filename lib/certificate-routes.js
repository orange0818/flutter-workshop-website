...
export function createCertificateHandlers({ rootDir, certificateData, logAccess }) {
  async function loadCertificateOrRespond(student, res, clientIP, startTime) {
    ...
    const certPath = getCertificatePath(rootDir, student.name, certificateData.certificateFiles);
    const buffer = readCertificateBuffer(certPath);
    if (!buffer) {
      logAccess(student.name, 'REJECTED - file_not_found', clientIP, Date.now() - startTime);
  ...
  return {
    ...
    async preview(req, res) {
      ...
      const assignmentCompleted = certificateData.hasCompletedAssignment(student.name);
      const buffer = await loadCertificateOrRespond(student, res, clientIP, startTime);
      if (!buffer) return;
